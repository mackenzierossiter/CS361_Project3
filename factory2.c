...
int main(int argc, char *argv[])
{
    ...
    while (1) {
        ...
        //receive from client
        // how do we specifcally wait for a PRODUCTION_MSG message?
        if ( recvfrom( sd , buf , MAXBUFLEN , 0 , (SA *) & clntSkt , & alen ) < 0 )
            err_sys( "recvfrom" ) ;
            
        //got lost here
        if (buf->purpose == REQUEST_MSG)
        {
            fprintf( stderr, "FACTORY server received: { %s, OrderSize= %s}", buf, buf->orderSize) ;
        }
